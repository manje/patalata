...
<div class="w-full h-64 relative">
  ...
  @foreach ($articles[0]->apfiles as $file)
  ...
  <div class="absolute inset-0 bg-black bg-opacity-50 flex flex-col justify-center px-4">
    <!-- Título del post -->
    <a href="{{ route('articles.show', $articles[0]->slug) }}" class="text-white font-bold text-4xl hover:underline">
      {{ $articles[0]->name }}
    </a>
    <div class="text-white text-sm">
      <span>{{ $articles[0]->created_at->diffForHumans() }}</span>
      @if ($articles[0]->place)
         - <span> {{ $articles[0]->place->name }} </span>
      @endif
    </div>
  </div>
</div>
...
<div class="flex flex-wrap">
    @foreach ($articles->slice(1, 4) as $index => $post)
      <div class="w-full h-64 relative md:w-1/2 lg:w-1/4 bg-gray-400 hidden
        ...
        <div class="absolute inset-0 bg-black bg-opacity-50 flex flex-col justify-end p-4 px-4">
          <!-- Título del post -->
          <a href="{{ route('articles.show', $post->slug) }}" class="text-white font-bold text-4xl hover:underline">
            {{ $post->name }}
          </a>
          <div class="text-white text-sm">
